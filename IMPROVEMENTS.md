...
---
        ...
        **現状の問題**
        ...
        ### 3-B. 買い物リストのアーカイブ機能 →対応済み（2026-02-26）
...
---
        ...
        ### 3-C. 費用管理のカテゴリ別集計強化 →対応済み（2026-02-26）
        ...
        **改善案**
        ...
        ### 3-D. CSVエクスポートの完成 →対応済み（2026-02-26）
...
## 4. UX・デザイン（優先度：中）
        ...
        ### 4-A. ローディング状態の統一 →対応済み（2026-02-26）
        ...
        **対応内容**
        ...
        - `src/components/Loading.tsx` に共通 `<Loading />` と `PageSkeleton` を実装
        - `src/components/RequestStatus.tsx` の `LoadingNotice` を共通 `<Loading />` 利用に統一
        - `src/app/loading.tsx` を追加し、初回表示で Skeleton UI を表示

---

### 4-B. エラーハンドリングのUI →対応済み（2026-02-26）
...
- トースト通知は既存の `ToastViewport` / `showToast` で継続利用
- `src/components/RequestStatus.tsx` に `RetryNotice` を追加
- `src/app/error.tsx` を追加し、画面読み込み失敗時の再試行導線を標準化
- `src/app/settings/page.tsx` の共益費設定初期取得失敗時に「再取得」ボタンを追加
