...
| 中 | TASK-OPT3 | 重複ロジックのカスタムフック化 | 完了 | 7 |
| 低 | TASK-OPT4 | loading.tsx の拡充 | 完了 | 7 |
...
複数のフック間で以下のパターンが繰り返し実装されていた。

- フォーム送信時の `submitting` / `errorMessage` 管理（4つのフォームモーダル + ルール編集）
- ID 追跡付き非同期アクション（`setXxxId(id)` → try/finally → `setXxxId(null)`）（削除・取消・確認処理）
- 展開/折り畳みリスト（`isExpanded` + `items.slice(0, limit)`）

**実施内容**

1. `src/hooks/useFormSubmit.ts` を作成 — `submitting` / `errorMessage` / `handleSubmit` を共通化
2. `src/hooks/useItemAction.ts` を作成 — `activeId` / `execute(id, action)` を共通化
3. `src/hooks/useExpandableList.ts` を作成 — `isExpanded` / `visibleItems` / `hasMore` を共通化
4. 以下の既存フックをリファクタリングして新フックを適用:
   - `useExpenseFormModal`, `useShoppingFormModal`, `useNoticeFormModal`, `useRuleFormModal` → `useFormSubmit`
   - `useNoticesSection`, `useRulesSection`, `useExpenseSection`, `useShoppingSection` → `useItemAction`
   - `useRulesSection` の saveEdit → `useFormSubmit`
   - `useExpenseSection` の履歴・調整リスト → `useExpandableList`

**受け入れ基準**
- 重複していたステート管理パターンが共通フックに集約されている
- TypeScript 型チェック・Next.js ビルドがパスする
- 各画面の動作に変更がない
...
以下の5ルートにページ固有のスケルトン UI（`loading.tsx`）を追加:

1. `src/app/tasks/loading.tsx` — 急ぎのタスク・貢献度カルーセル・最近の完了のスケルトン
2. `src/app/shopping/loading.tsx` — 未購入リスト・購入済みリストのスケルトン
3. `src/app/notices/loading.tsx` — 重要お知らせ・通常お知らせのスケルトン
4. `src/app/rules/loading.tsx` — 未確認ルール・カテゴリ別ルールのスケルトン
5. `src/app/settings/loading.tsx` — 通知設定・トグル・各種セクションのスケルトン
