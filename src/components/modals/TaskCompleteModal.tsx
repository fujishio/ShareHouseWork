...
import { useTaskCompleteModal } from "@/hooks/useTaskCompleteModal";
    ...
    <>
      <div className="flex items-center justify-between px-4 pt-3 pb-2 border-b border-stone-100 shrink-0">
        <div className="flex items-center gap-1">
          ...
          <h2 className="text-sm font-bold text-stone-800">{selectedCategory ?? "ジャンル選択"}</h2>
        </div>
      </div>
          ...
          <div className="space-y-1.5">
            {categories.map((category) => (
              ...
              >
                ...
                <span className="text-xs text-stone-400">{category.count}件</span>
              </button>
            ))}
          </div>
        ) : (
          <div className="space-y-1.5">
            {filteredTasks.map((task) => {
              ...
              return (
                <button
                  key={task.id}
                  onClick={() => !completedTaskId && void completeTask(task.id)}
                ...
                >
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${
                        isCompleted ? "bg-amber-500 text-white scale-110" : "bg-stone-200 text-stone-400"
                      }`}
                    >
                      <Check size={12} strokeWidth={2.5} />
                    </div>
                    <span className={`text-xs font-medium ${isCompleted ? "text-amber-700" : "text-stone-700"}`}>
                  ...
                  <span
                    className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex items-center gap-0.5 shrink-0 ${
                      isCompleted ? "bg-amber-200 text-amber-800" : "bg-amber-100 text-amber-700"
                    }`}
                  >
